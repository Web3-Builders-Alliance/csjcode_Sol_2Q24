...
import type { EscrowDoerz } from '../target/types/escrow';

// Explicitly import from the same instance
...
describe('escrow', async () => {
  ...
  // Explicitly cast the connection to the expected type
  const connection = provider.connection as Connection;

  const program = anchor.workspace.Escrow as Program<EscrowDoerz>;
